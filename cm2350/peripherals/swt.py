...
from ..intc_exc import MceDataReadBusError, MceWriteBusError, ExternalException, ResetException, INTC_SRC
...
class SWT(MMIOPeripheral):
    ...
    def __init__(self, emu, mmio_addr):
        ...
        super().__init__(emu, 'SWT', mmio_addr, 0x4000, regsetcls=SWT_REGISTERS)
    ...
    def handleTimeout(self):
        ...
        with self._wdogHandlerLock:
            if self.registers.mcr.itr and not self._timeout_count:
                ...
                self.registers.ir.vsOverrideValue('tif', 1)

                # Queue the exception so it is processed before the next
                # instruction is executed.  This queues a SWT peripheral
                # interrupt, not the PPC core WatchdogTimer interrupt (which is
                # only generated by the MCU watchdog)
                self.emu.queueException(ExternalException(INTC_SRC.SWT))
            else:
